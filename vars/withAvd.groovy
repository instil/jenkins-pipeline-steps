...
private def launchAvd(String avdName) {
    ...
    timeout(time: 5, unit: "MINUTES") {
        sh "$ANDROID_HOME/emulator/emulator -avd $avdName -port $emulatorPort -memory 2048 -partition-size 4096 -netfast -accel auto -gpu auto -no-window -no-boot-anim -no-audio -no-snapshot &"
